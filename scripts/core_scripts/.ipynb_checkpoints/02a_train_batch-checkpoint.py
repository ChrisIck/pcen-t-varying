...
if __name__ == '__main__':
    ...
    if params.validation_size == None:
        val_size = 6000 * len(feature_list)
    else:
        val_size = params.validation_size
    ...
    gen_val = data_generator_val(train_features, sampler_val, random_state=params.seed, slices=slices,\
                                 batch_size=params.batch_size)
    '''
    print('Data Gen')
    data_dict = next(gen_train)
    print(data_dict['PCEN/mag'].shape)
    print(data_dict['dynamic/tags'].shape)
    '''
    
    print("Constructing model...")
    construct_model = MODELS[params.modelname]
    
    input_layer = Input(name=field,  shape=(None, params.n_mels, len(slices)),\
                              dtype='float32')    
    ...
    '''
    print('Keras Gen')
    test_data, test_labs = next(gen_train)
    print(np.array(test_data).shape)
    print(np.array(test_labs).shape)
    '''
    ...
    '''
    print('Label Gen')
    test_data, test_labs = next(gen_train_label)
    print(np.array(test_data).shape)
    print(np.array(test_labs).shape)
    '''
    ...
    history = model.fit(gen_train_label, epochs=params.epochs, steps_per_epoch=params.epoch_size, 
